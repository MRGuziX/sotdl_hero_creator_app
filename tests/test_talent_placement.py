...
from utils.pdf_creator import distribute_talents, get_talent_boxes
...
def test_talent_distribution():
    ...
    tradition = Talent(name="Tradycja: Magia Niebiańska", description="Dostęp do zaklęć")
    
    talents = [determinacja, wspolna_odnowa, medium_talent, huge_talent, small_1, small_2, small_3, small_4, tradition]
    ...
    for box_id, data in assigned.items():
        print(f"Box {box_id} ({data['box'].capacity} chars): {data['name']} (desc len: {len(data['description'] if data['description'] else '')})")
